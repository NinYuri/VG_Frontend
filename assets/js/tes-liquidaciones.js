...
// Cards Information
function renderCards(liquidaciones) {
...
function llenarInfoCard(card, data) {
    ...
    if(data.fecha_recibido)
        ...
        if(data.tipo_ajuste === 'Reembolso')
            fechaRecibido = 'Pendiente de confirmación por el colaborador';
...
async function buttonTransfer() {
    document.body.addEventListener('click', async(e) => {
        ...
        if(ajuste === 'Reembolso')
            ToastRefund('SELECCIONA EL MÉTODO DE PAGO', '¿Cómo deseas realizar el pago correspondiente a esta liquidación?', 'EFECTIVO', 'TRANSFERENCIA', solicitud);
        else
            ToastReturn(solicitud);
    });
}
...
function initReceiptUpload() {
    ...
    async function uploadAndConfirm(solicitud) {
        ...
        try {
            ...
            Toast('COMPROBANTE CARGADO', '¡Listo! Hemos notificado al colaborador para que confirme la recepción de su reembolso');
        ...
        try {
            ...
            if(data.ruta_comprobacion) {
                ...
                if(data.fecha_recibido)
                    ...
                    if(ajuste === 'Devolución')
                        document.querySelector('.transfer-confir span').innerHTML = 'Pendiente de tu confirmación';
                    else
                        document.querySelector('.transfer-confir span').innerHTML = 'Pendiente de confirmación por el colaborador';
                }
                ...
                if(data.fecha_recibido)
                    ...
                    if(ajuste === 'Devolución')
                        ...
                        document.querySelector('.cash-confir span').innerHTML = 'Pendiente de confirmación por el colaborador';
                }
            }
...
// Toast -> Buttons
function ToastRefund(title, message, cancel, confirm, sol) {
    Swal.fire({
        title: title,
        html: `
...
function confirmMoney(sol) {
    Swal.fire({
        ...
        html: `
            <img src="./assets/images/Icon_agave1.webp" alt="Agave" class="agave-half left">
            <img src="./assets/images/Icon_agave2.webp" alt="Agave" class="agave-half right">
            <p class="received-text">¿Se ha entregado el reembolso en efectivo al colaborador para la liquidación de la solicitud ${sol}?</p>
        `,
        position: 'top-end',
        background: '#00333E',
        color: '#FFFFFF',
        ...
        if(result.isConfirmed) {
            try {
                ...
                Toast('REEMBOLSO REGISTRADO', '¡Listo! Hemos notificado al colaborador para que confirme la recepción de su reembolso');
...
function ToastReturn(sol) {
    Swal.fire({
        title: 'CONFIRMAR RECEPCIÓN DE DEVOLUCIÓN',
        html: `
            <img src="./assets/images/Icon_agave1.webp" alt="Agave" class="agave-half left">
            <img src="./assets/images/Icon_agave2.webp" alt="Agave" class="agave-half right">
            <p class="received-text">¿Has recibido la devolución correspondiente a la liquidación de la solicitud ${sol}?</p>
        `,
        position: 'top-end',
        background: '#00333E',
        color: '#FFFFFF',
        ...
        try {
            if(result.isConfirmed) {
                ...
                Toast('RECEPCIÓN DE DEVOLUCIÓN', '¡Listo! La devolución ha sido confirmado correctamente');
            } else if (result.dismiss === Swal.DismissReason.cancel) {
